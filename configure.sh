...
#      of the project will be generated in the directory chi_build. Additional
...
#      scripts for the corresponding build of the project will be generated in
...
#      regenerate the CMake scripts for a Release build of the project from scratch
#      (invalidating CMake cache that stores previously used settings)
#
#   ./configure.sh -DCMAKE_VERBOSE_MAKEFILE=1
#      generate the CMake scripts for a Release build of the project (using CMake
...
#      generate the CMake scripts for a Debug build of the project (using CMake
#      cache if available); the 'make' command will place the project executable
...
if [ $DO_CMAKE = "Yes" ]; then
  mkdir -p "${PROJECT_NAME}_build" && cd "${PROJECT_NAME}_build" && \
