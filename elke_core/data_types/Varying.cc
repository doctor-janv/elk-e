...
void elke::Varying::CheckTypeMatch(
...
elke::Varying&
elke::Varying::operator=(const Varying& other)
{
  if (this != &other)
  {
    data_ = other.data_->Clone();
...
elke::Varying&
elke::Varying::operator=(const std::vector<std::byte>& value)
{
  type_ = VaryingDataType::ARBITRARY_BYTES;
  data_ = std::make_unique<VaryingArbitraryType<std::vector<std::byte>>>(value);
  return *this;
...
elke::Varying&
elke::Varying::operator=(const std::string& value)
{
  type_ = VaryingDataType::STRING;
  data_ = std::make_unique<VaryingArbitraryType<std::string>>(value);
  return *this;
...
size_t elke::Varying::ByteSize() const { return data_->Size(); }

// ###################################################################
/**Returns a string value for the value.*/
std::string elke::Varying::PrintStr(bool with_type/*=false*/) const
{
  std::stringstream outstr;

  if (this->Type() == elke::VaryingDataType::STRING)
    outstr << "\"" << this->StringValue() << "\"";
  else if (this->Type() == elke::VaryingDataType::FLOAT)
    outstr << this->FloatValue() << (with_type ? "(double)" : "");
  else if (this->Type() == elke::VaryingDataType::INTEGER)
    outstr << this->IntegerValue();
  else if (this->Type() == elke::VaryingDataType::BOOL)
