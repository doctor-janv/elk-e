...
parser = argparse.ArgumentParser(
    description="This is a utility script for the download and installation of "
                + "dependencies.",
...
# Gets an EnvVar or sets it if not defined
def GetSetEnvVar(env_var_name: str, default_value: str) -> str:
    value = os.getenv("CC")
    if value is None or len(value) == 0:
        return default_value
    return value

# Gets an EnvVar and raises an exception if it is not found
def GetEnvVar(env_var_name: str) -> str:
    value = os.getenv(env_var_name)
    if value is None or len(value) == 0:
        log_file.write(f"EnvVar \"{env_var_name}\" not found")
        raise RuntimeError(f"EnvVar \"{env_var_name}\" not found")
    else:
        log_file.write(f"EnvVar \"{env_var_name}\" found: {value}")
        return value
...
log_file.write("***** Dependency configurator script *****\n\n")
...
mpicc = GetEnvVar("CC")
mpicxx = GetEnvVar("CXX")
mpifort = GetEnvVar("FC")

print(mpicc)
CheckExecutableExists("C compiler", mpicc)
CheckExecutableExists("C++ compiler", mpicxx)
CheckExecutableExists("Fortran compiler", mpifort)
...
env_script_name = f"{install_dir}/bin/set_env.sh"
...
print("\nWhen opening this project in an IDE, the following environment variables need to be set:\n")
